...
class VirtualEnviroment:
    ...
    def calibrate_camera(self, camera_index):
        ...
        last_frame = self.calibration_info[camera_index][-1]

        true_pos, screen_pos = last_frame
        res = self.calibrate_single_frame(true_pos, screen_pos, camera_index)

        if res is not None:
            print(f"Camera position: {res[0]}")
